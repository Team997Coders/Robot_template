...
public class Drive extends SubsystemBase implements Vision.VisionConsumer {
    ...
    public void periodic() {
        for (int i = 0; i < 4; i++) {
            SmartDashboard.putNumber(
                    "Module number " + i + " Turn relative Encoder", modules[i].getTurnRelativeEncoder());
            SmartDashboard.putNumber(
                    "Module number " + i + " Turn absolute Encoder", modules[i].getTurnAbsoluteEncoder());
        }
    ...
    // Offsets swerve wheels by offset
    public void runVelocityOffsets(ChassisSpeeds speeds, double angleOffsets) {
        // Calculate module setpoints
        speeds = ChassisSpeeds.discretize(speeds, 0.02);
        SwerveModuleState[] setpointStates = kinematics.toSwerveModuleStates(speeds);
        SwerveDriveKinematics.desaturateWheelSpeeds(setpointStates, maxSpeedMetersPerSec);
        ...
        // Send setpoints to modules
        for (int i = 0; i < 4; i++) {
            modules[i].runSetpointOffsets(setpointStates[i], angleOffsets);
        }
